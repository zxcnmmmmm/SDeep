...
from models.utils import dummy_context_mgr
...
class CLIP_Adapter(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, G_ch, CLIP_ch, cond_dim, k, s, p, map_num, CLIP):
        ...
        self.SD_M_Blocks = nn.ModuleList([])
        self.SD_M_Blocks.append(SD_M_Block(in_ch, mid_ch, out_ch, cond_dim, k, s, p))
        for i in range(map_num-1):
            self.SD_M_Blocks.append(SD_M_Block(out_ch, mid_ch, out_ch, cond_dim, k, s, p))
    ...
    def forward(self,out,c):
        ...
        for SD_M_Block in self.SD_M_Blocks:
            out = SD_M_Block(out,c)
...
class NetG(nn.Module):
    def __init__(self, ngf, nz, cond_dim, imsize, ch_size, mixed_precision, CLIP):
        ...
        for idx, (in_ch, out_ch) in enumerate(in_out_pairs):
            if idx<(len(in_out_pairs)-1):
                ...
                imsize = 224
            self.SDBlocks.append(SDBlock(cond_dim+nz, in_ch, out_ch, imsize))
        self.mlca = MLCAttention(kernel_size=3)
    ...
    def forward(self, noise, c, eval=False):
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else dummy_context_mgr() as mp:
            ...
            for SDBlock in self.SDBlocks:
                out = SDBlock(out, cond)
            out = self.mlca(out)
            out = self.to_rgb(out)
        return out
...
class NetD(nn.Module):
    def __init__(self, ndf, imsize, ch_size, mixed_precision):
        ...
        self.Dis_Blocks = nn.ModuleList([
            Dis_Block(768, 768, 3, 1, 1, res=True, CLIP_feat=True),
            Dis_Block(768, 768, 3, 1, 1, res=True, CLIP_feat=True),
        ])
        self.main = Dis_Block(768, 512, 3, 1, 1, res=True, CLIP_feat=False)

    def forward(self, h):
        with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
            out = h[:,0]
            for idx in range(len(self.DBlocks)):
                out = self.Dis_Blocks[idx](out, h[:,idx+1])
            out = self.main(out)
        return out
...
class SDLayer(nn.Module):
    def __init__(self, cond_dim, in_ch):
        ...
        self.affine_0 = Affine(cond_dim, in_ch)
        self.affine_1 = Affine(cond_dim, in_ch)
        self.affine_2 = Affine(cond_dim, in_ch)
...
class SDBlock(nn.Module):
    def __init__(self, cond_dim, in_ch, out_ch, imsize):
        ...
        self.fuse1 = SDLayer(cond_dim, in_ch)
        self.fuse2 = SDLayer(cond_dim, out_ch)
...
class MLCAAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        y = torch.cat([max_result, avg_result], 1)
        y = self.gap(x)  # bs,c,1,1
